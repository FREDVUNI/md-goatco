...
function validateStep(step) {
  ...
  if (!valid) {
    ...
    showStepError(step, 'Please fill in all required fields before continuing.');
  } else {
    clearStepError(step);
  }
...
document.querySelectorAll('.file-input').forEach(input => {
  input.addEventListener('change', function () {
    ...
    if (file.size > 5 * 1024 * 1024) {
      preview.innerHTML = '<span class="file-name" style="color:var(--red)">⚠ File too large (max 5 MB)</span>';
      this.value = '';
      return;
    }
...
function initPasswordStrength() {
  ...
  bar.style.cssText = 'height:3px;border-radius:2px;margin-top:6px;transition:width .3s ease,background .3s ease;width:0;';
  const label = document.createElement('p');
  label.style.cssText = 'font-size:0.72rem;margin-top:4px;color:var(--slate-light);';
