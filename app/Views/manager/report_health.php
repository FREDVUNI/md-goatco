...
<div class="card">
  <div class="card-head">
    <h3>Health Report — Active Flags</h3>
    <a href="<?= site_url('manager/reports/export/health') ?>" class="btn btn-ghost btn-sm">📥 Export</a>
  </div>

  <?php if (empty($flags)): ?>
  <div class="empty-state">✅ No active health flags — the herd is looking good.</div>
  <?php else: ?>
  <table>
    <thead>
      <tr><th>Animal</th><th>Owner</th><th>Flagged</th><th>Reason</th><th>Vet</th></tr>
    </thead>
    <tbody>
      <?php foreach ($flags as $f): ?>
      <tr>
        <td><a href="<?= site_url('manager/herd/' . $f['goat_id']) ?>" style="font-weight:600;color:var(--blue)"><?= esc($f['goat_name'] ?? '') ?></a> <span class="tag"><?= esc($f['tag_number'] ?? '') ?></span></td>
        <td><?= !empty($f['member_first']) ? esc($f['member_first'] . ' ' . $f['member_last']) : '—' ?></td>
        <td><?= date('j M Y', strtotime($f['created_at'])) ?></td>
        <td><?= esc(substr($f['flag_reason'] ?? '', 0, 60)) ?></td>
        <td><?= esc(($f['vet_first'] ?? '') . ' ' . ($f['vet_last'] ?? '')) ?></td>
      </tr>
      <?php endforeach ?>
    </tbody>
  </table>
