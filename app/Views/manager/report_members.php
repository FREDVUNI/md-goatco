...
<div class="card">
  <div class="card-head">
    ...
    <a href="<?= site_url('manager/reports/export/members') ?>" class="btn btn-ghost btn-sm">📥 Export</a>
  </div>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        <td><a href="<?= site_url('manager/members/' . $m['id']) ?>" style="font-weight:600;color:var(--blue)"><?= esc($m['first_name'] . ' ' . $m['last_name']) ?></a></td>
        <td><?= esc($m['email']) ?></td>
        <td><?= esc($m['phone'] ?: '—') ?></td>
