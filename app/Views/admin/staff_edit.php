...
<div class="card" style="max-width:600px">
  <div class="card-head"><h3>Edit Staff — <?= esc($staff['first_name'] . ' ' . $staff['last_name']) ?></h3></div>
  ...
  <?= form_open('admin/staff/' . $staff['id'] . '/edit', ['class' => 'dash-form']) ?>
    <?= csrf_field() ?>
    ...
    <div class="field-row">
      <div class="field">
        ...
        <input type="text" id="first_name" name="first_name"
               value="<?= esc(old('first_name') ?? $staff['first_name']) ?>" required>
      </div>
      <div class="field">
        <label for="last_name">Last name *</label>
        <input type="text" id="last_name" name="last_name"
               value="<?= esc(old('last_name') ?? $staff['last_name']) ?>" required>
      </div>
    </div>
    <div class="field">
    ...
    <div class="field">
      ...
      <input type="tel" id="phone" name="phone"
             value="<?= esc(old('phone') ?? $staff['phone'] ?? '') ?>" placeholder="+256 700 000 000">
    </div>
    ...
    <div class="field">
      ...
      <select id="role" name="role" required>
        <option value="vet"     <?= ($staff['role'] === 'vet') ? 'selected' : '' ?>>Veterinarian — Can log vet visits, flag animals, view herd</option>
        <option value="manager" <?= ($staff['role'] === 'manager') ? 'selected' : '' ?>>Farm Manager — Full herd management, vet scheduling, reports</option>
      </select>
    </div>

    <div class="form-section-label">Reset password (optional)</div>
    <div class="field">
      <label for="new_password">New password</label>
      <input type="password" id="new_password" name="new_password" placeholder="Leave blank to keep current password">
      <p class="field-hint">Only fill this in if you want to manually set a new password. Otherwise, use the "Reset password" button on the staff list to generate and email one automatically.</p>
    </div>
